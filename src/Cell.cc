	...
	for (s=0; s<nr_symbionts; s++)
	{
		...
		while (ir != Symbionts->at(s)->G->ExpressedGenes->end())
		{
			...
			{	//Protein translocated to host.
				ir2 = ir;
				ir--;
				if (communication_to_host || uniform() < leakage_to_host)	//Proteins allowed to enter the host.
				{
			...
			else if ( (((*ir)->signalp.test(0) && (*ir)->signalp.test(1)) && communication_to_host ) || uniform() < leakage_to_host)
		...
		while (it_cntr < Host->G->nr_native_expressed)
		{
			...
			{	//Protein translocated to symbiont.
				if (s == nr_symbionts-1)	//Only erase the expressed gene from the host if we get to the last symbiont (already transported to all other symbionts).
				{
					...
					if (communication_to_symbiont || uniform() < leakage_to_symbiont)
					{
			...
			else if ( (((*ir)->signalp.test(0) && (*ir)->signalp.test(1)) && communication_to_symbiont) || uniform() < leakage_to_symbiont)
			{
				Symbionts->at(s)->G->ExpressedGenes->push_back(*ir);
			}
			ir++;
...
{
	if (Symbionts->at(s)->mutant || trace_lineage || log_lineage || log_site)
	{
...
{
	if (Host->mutant || trace_lineage || log_lineage || log_site)
	{
		Host->alive = false;
	}
	else
